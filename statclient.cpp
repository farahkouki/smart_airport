...
QChartView * statclient::Preparechart(QString type,QString par)
{
    QSqlQuery query;
    int count1=0;
    int count2=0;
...
int max;
    ...
    { int count4=0;
      ...
      int count8=0;
      int count9=0;
    if(query.exec("select adresse from Clients"))
    {
    while(query.next())
      {
    ...
    chart->setTitle("Les adresses");
    chartView = new QChartView(chart);
    chartView->setRenderHint(QPainter::Antialiasing);
    }
    else if(type=="BARRE"){

        maxcount=qMax(count1,count2);
        maxcount=qMax(maxcount,count3);
        maxcount=qMax(maxcount,count4);
        maxcount=qMax(maxcount,count5);
        maxcount=qMax(maxcount,count6);
        maxcount=qMax(maxcount,count7);
        maxcount=qMax(maxcount,count8);
        maxcount=qMax(maxcount,count9);
